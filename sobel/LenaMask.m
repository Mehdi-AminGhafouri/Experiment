...
% because we use this mask for calculating value of the center pixel of mask
% we will loose 2 pixel from each side top/bottom/left/right
...
figure(2);
subplot(1,2,1)
imshow(uint8(abs(lenaA)));
title('horizontal');
subplot(1,2,2)
imshow(uint8(abs(lenaB)));
title('vertical');
